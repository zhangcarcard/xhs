...
<head>
    ...
    <#include "/meta.ftl">
    <@common.title name="RANDOM转发配置" />
    <#include "/head.ftl">
</head>
<body>
<#include "/fileupload.ftl">
<div class="container">
    <@common.nav menu="loadbalance" submenu="random" />
    ...
    <div id="toolbar" class="btn-group">
        ...
        <a type="button" class="btn btn-default" href="/file/template/random.do">
...
<script>
    $(function() {
        createBootstrapTable('#table', '/loadbalance/random/random.do', ['ip', 'port', 'protocalName', 'groupName'], ['IP', 'PORT', '协议名称', '分组名称'], true, '#toolbar', 'GET');
        createFileInput('#pupload', '/file/template/random.do', function (event, data, previewId, index) {
        ...
        $("#btn_delete").click(function() {
            ...
            if (selections.length == 0) {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            url: "/loadbalance/random/random.do",
                            type: "DELETE",
        ...
        $("#form").submit(function() {
            ...
            $.post("/loadbalance/random/random.do", params, function(result) {
                if (result.success) {
                    toastr.success('保存成功.');
                    $('#table').bootstrapTable('refresh', {silent: true});
                    $('#addModal').modal('hide');
